// Start of the automatic chain: linalg.matmul (bf16 * bf16 -> f32).
// Transform: pack K into VNNI pairs of 2, tile to one zmm, vectorize.
// Mixed types: C is f32 so the packed-dot pattern can match.
module attributes {transform.with_named_sequence} {
  ...
  transform.named_sequence @__transform_main(
    ...
    // Pack only K into VNNI pairs. packed_sizes is [M, N, K].
    %packed = transform.structured.pack %mm packed_sizes = [0, 0, 2]
        : (!transform.any_op) -> (!transform.op<"linalg.generic">)

    // N outer (B panel reused across rows), M=1, K leftover as a loop.
