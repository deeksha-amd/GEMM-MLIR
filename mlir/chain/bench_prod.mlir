...
module attributes {transform.with_named_sequence} {
  ...
  func.func @matmul(%A: tensor<128x2048x2xbf16>,
                    %B: tensor<2048x4096x2xbf16>,
                    %C: tensor<128x4096xf32>) -> tensor<128x4096xf32> {
    %0 = linalg.generic {indexing_maps = [#map, #map1, #map2],
                         iterator_types = ["parallel", "parallel", "reduction", "reduction"]}
        ins(%A, %B : tensor<128x2048x2xbf16>, tensor<2048x4096x2xbf16>)
        outs(%C : tensor<128x4096xf32>) {
  ...
  func.func @main() {
    ...
    %Ae = tensor.empty() : tensor<128x2048x2xbf16>
    %Be = tensor.empty() : tensor<2048x4096x2xbf16>
    %Ce = tensor.empty() : tensor<128x4096xf32>
    %A = linalg.fill ins(%one : bf16) outs(%Ae : tensor<128x2048x2xbf16>) -> tensor<128x2048x2xbf16>
    %B = linalg.fill ins(%one : bf16) outs(%Be : tensor<2048x4096x2xbf16>) -> tensor<2048x4096x2xbf16>
    %C0 = linalg.fill ins(%zf : f32) outs(%Ce : tensor<128x4096xf32>) -> tensor<128x4096xf32>
    %Cw = func.call @matmul(%A, %B, %C0)
        : (tensor<128x2048x2xbf16>, tensor<2048x4096x2xbf16>, tensor<128x4096xf32>) -> tensor<128x4096xf32>
    %t0 = func.call @rtclock() : () -> f64
    %Cr = scf.for %r = %c0 to %reps step %c1
        iter_args(%acc = %Cw) -> (tensor<128x4096xf32>) {
      %t = func.call @matmul(%A, %B, %acc)
          : (tensor<128x2048x2xbf16>, tensor<2048x4096x2xbf16>, tensor<128x4096xf32>) -> tensor<128x4096xf32>
      scf.yield %t : tensor<128x4096xf32>
    }
    %t1 = func.call @rtclock() : () -> f64
    %dt = arith.subf %t1, %t0 : f64
    func.call @printF64(%dt) : (f64) -> ()
    func.call @printNewline() : () -> ()
    %v = tensor.extract %Cr[%c0, %c0] : tensor<128x4096xf32>
  ...
  transform.named_sequence @__transform_main(%root: !transform.any_op {transform.readonly}) {
    ...
    // N outer so one 16-wide B panel is reused across rows.
    ...
    // Keep the 1x16 C tile in a vector iter_arg across K (no load/store per k).
